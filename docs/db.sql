...
create table UserDebitOut(
       ...
       udo_type     varchar(100),            --放贷类型          暂定              以后用的时候再说 
	   temp varchar(100) default null,
	   temp varchar(100) default null,
	   temp varchar(100) default null       
...
create table AttentionMark(
	...
	udi_id  		int, --借贷表id外键
	am_time 		date,   --关注时间
	am_status	int, 	--关注状态   0代表取消关注   1代表正在关注			修改
...
create table UserMessage(
       ...
       um_idCard     	int,	   --身份证    只能为18位数字
       um_sex       	 int,	         --性别    年龄     出生日期    居住地    都通过身份证获取
       um_age         int,
       um_birthday  date,                  --出生日期 
       um_nowPlace varchar(100),   --居住地
	   um_workinfo    varchar(100),     --工作信息
	   um_incomeinfo  varchar(100),     --收入信息
       um_image     text  default null,     --图像
	   temp varchar(100) default null,
	   temp varchar(100) default null,
	   temp varchar(100) default null,
	   temp varchar(100) default null,
	   temp varchar(100) default null	   --资料就先写这么多，像人人贷一样,到时候可以接一些接口,个人征信报告，微粒贷记录
...
create table bankInfo(
	bi_id int ,		
	bi_name,					--银行卡持有人姓名
	bi_idCard varchar(20) unique,		--银行卡号
	u_id int ,					--用户id    外键约束
	bi_status int 				--银行卡状态（是否在用）   0表示不可用  1表示可用
	temp varchar(100) default null,
	temp varchar(100) default null,
	temp varchar(100) default null
)
